<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMPdtBrand))" %>
...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaMenuPdtBrand") = Resources.resGBGridHeader.olaMenuPdtBrand
        ViewData("olaFindPdtBrand") = Resources.resGBGridHeader.olaFindPdtBrand
        ViewData("olaEdit") = Resources.resGBGridHeader.olaEdit
        ViewData("olaPdtBrandList") = Resources.resGBGridHeader.olaPdtBrandList
        ...
        ViewData("olaMenuGroup") = Resources.resGBMenuName.omnMngGenPdt
...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
	<%=ViewData("olaPdtBrandList")%>
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
<input type="hidden" id="otbFilterField"  />
<input type="hidden" id="otbtextField" />   
<%--<div class="xW_DivMain xCN_AllBackgroundColor xCN_AllBorderColor xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor">--%>
<div class="xCN_DivMain xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor" >
 <%--   <div class="xW_DivToppic xCN_AllMenuFontSize">
         <label><%=ViewData("olaMenuPdtBrand")%></label>         
    </div>--%>
    <%--<div class="xW_DivToppic xCN_AllMenuFontSize">
        <label>\\<a class="xCN_LabelLink" href="<%=Url.Action("Index","Home") %>"><%=ViewData("olaMenuHome") %></a>\<a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt"><%=ViewData("olaMenuGroup") %></a>\<%=ViewData("olaMenuPdtBrand")%></label> 
    </div>--%>
    <%--\\<a href="<%=Url.Action("Index","Home") %>">หน้าหลัก</a>\<a href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt">สินค้า</a>\<%=ViewData("olaMenuPdtBrand")%>--%>
    ...
    <div class="xW_DivSearch">
        <div class='xW_DivPath'>
            <label>\\
                ...
                <a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt">
                    <%=ViewData("olaMenuGroup") %>
                </a>\
                <%=ViewData("olaMenuPdtBrand")%>
            </label>
        </div>
        <div class ="xW_DivSearchCapPrd">
            <label id="olaFilterCaption"><%=ViewData("olaFindPdtBrand")%></label>
        </div>
...
<%-------------------เมนูด้านบนสุด---------------------------%>
    ...
    <% 
        Dim gridBuilder = Html.Telerik().Grid(Of Adasoft.WebPos.Models.cmlTCNMPdtBrand)(Model)
        gridBuilder.Name("Grid")
        gridBuilder.Pageable(Function(page) page.PageSize(15))
        '*ปิดไว้ ใช้งานปุ่มที่สร้างเอง *CH 18-10-2012
        ...
        gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
                  .Select("_SelectAjaxEditing", "cvcPdtBrand") _
                  .Update("_SaveAjaxEditing", "cvcPdtBrand") _
                  .Delete("_DeleteAjaxEditing", "cvcPdtBrand", New With {.id = (Function(o) o.FTPbnCode)}) _
                  .Insert("_InsertAjaxEditing", "cvcPdtBrand")))
     
            'Add grid columns
            'gridBuilder.Columns(Function(columns) columns.Add(Function(o) o.RowID).Width(100))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPbnCode).Width(150).Title(ViewData("olaCode")) _
                                .HeaderHtmlAttributes(New With {.name = "FTPbnCode", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPbnName).Width(503).Title(ViewData("olaName")) _
                                .HeaderHtmlAttributes(New With {.name = "FTPbnName", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
          
        gridBuilder.DataKeys(Function(keys) (keys.Add(Function(c) c.FTPbnCode)))
        ...
        Dim commandColumn As New GridActionColumn(Of Adasoft.WebPos.Models.cmlTCNMPdtBrand)(gridBuilder.ToComponent())
        ...
        gridBuilder.RowAction(Function(row) (row.Selected = row.DataItem.FTPbnCode.Equals(ViewData("id"))))
...
<script type="text/javascript">
    ...
    function W_CHKxStatus(e) {
        ...
        tW_CmdMode = e.mode;
        document.getElementById('FTPbnCode').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        document.getElementById('FTPbnName').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        if (tW_CmdMode == 'edit') {
            J002_SETxReadOnly("FTPbnCode", true);
            document.getElementById("FTPbnCode").onkeydown = function() { return false; };
            document.getElementById('FTPbnName').focus();
            document.getElementById('FTPbnName').select();
        } else {
            document.getElementById('FTPbnCode').focus();
        }
    }
    ...
    function W_CHKxUserAuthorize(e) {
        ...
        if (tW_CmdMode == 'insert') {
            ...
            if (tErrMsg != null && tErrMsg != '') {
                alert(tErrMsg);
                document.getElementById('FTPbnCode').focus();
                document.getElementById('FTPbnCode').select();
                return e.preventDefault();
            }
        }
    }
    function W_CHKxVerifyInsertData() {
        ...
        var tCode = document.getElementById("FTPbnCode").value;
        var tUrl = '<%=Url.Action("C_CHKtVerifyInsertData","cvcPdtBrand") %>' + "?ptDateTimeNow=" + tDataTimeNow + "&ptCode=" + tCode;
    ...
    function W_CHKxUserDeleteAuthorize(e) {
        ...
        if (document.getElementById('FTPbnCode') != null) {
            e.preventDefault();
            return false;
        }
    ...
    function W_SETxFilterOnClick() {
        ...
        $('#Grid tr:has(td) td', this.element).bind('click', function(e) {
            ...
            document.getElementById("olaFilterCaption").innerHTML = '<%=ViewData("olaFindPdtBrand")%>' + ' ' + tCaption;
            var tColName = $('#Grid th').eq(td.cellIndex).attr("name");
            document.getElementById('otbFilterField').value = tColName;
            //set caption
            document.getElementById('otbtextField').value = '<%=ViewData("olaFindPdtBrand")%>' + ' ' + tCaption;
        })
    }

    //เพิ่มเรคคอร์ดใหม่ใน Telerik //*CH 18-10-2012
     ...
     <script type ="text/javascript">
         ...
         function window_onload() {
             try {
                 ...
                 W_Show('<%=Viewdata("ptWheV")%>', 'FTPbnCode', '<%=ViewData("olaCode")%>');
                 J003_DISxWaiting();
             }
             catch (ex) {
                 J003_DISxWaiting();
         ...
         function W_Show(tValue, tFName, tCaption) {
             ...
             if (tCaption == "" || tCaption == null) {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtBrand")%>';
             }
             else {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtBrand")%>' + ' ' + tCaption;
         ...
         function W_FRMxSearchMaster() {
             ...
             if (oField.value != '') {
                 ...
                 //window.location = '<%=Url.Action("Index","cvcPdtBrand") %>' + '?ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value + '&ptAuthorRead=1';
             } else {
         ...
         function W_FRMxClose() {
             window.location = '<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt';
         }
    </script>

...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If tLastLang Is Nothing AndAlso tLastLang = "" Then
            ...
            AdaWebPos.My.Resources.resLCcvcTCNTCstGiftHD.Culture = New System.Globalization.CultureInfo(tLastLang)
        ...
        ViewData("olaGenCode") = AdaWebPos.My.Resources.resLCcvcGondola.olaGenCode
        ViewData("olaBrowse") = AdaWebPos.My.Resources.resLCcvcGondola.olaBrowse
        ...
        'Grid
        ViewData("olaSeqNo") = "Facing" 'AdaWebPos.My.Resources.resLCcvcTCNTCstGiftHD.olaSeqNo
        ...
        'Message
        Viewdata("tMsgNonData") = Resources.resGBMsg.tMsgNonData
        ViewData("tMsgNotDupFac") = resources.resgbmsg.tmsgnotdupfac
...
<script type="text/javascript">
    var tW_AddOrEdit = '1'; //1 Add; 2 Edit
    var tW_GonType; //รูปแบบของกอนโดล่า
    var tW_HavGonCode = '1'; //เก็บรหัสแสดงถึงว่ารหัสโดนใช้งานแล้วหรือยัง 0 Yes; 1 None
  ...
  function window_onload() {
      ...
      W_SETxDisplayTab(); //*CH 06-10-2012
      W_SETxRebindGrid();
  }
  ...
  function noenter(ptTarget) {
    ...
    function W_FRMxGenCode() {
        J001_GETtAutoGenCode('cvcGondola', 'C_Create_GetAutoCode', document.getElementById('FTGonCode'));
        J002_SETxReadOnly("FTGonCode", false);
        document.getElementById("FTGonCode").onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which, true); };

        var tCode = document.getElementById('FTGonCode').value;
        if (tCode == "Adasoft.Abreast.MaxValue") {
            document.getElementById('FTGonCode').value = "";
            document.getElementById('FTGonCode').focus();
            J003_SHWxWaiting();
    ...
    //*ตรวจสอบว่ามีรหัสกอนโดล่าอยู่แล้วหรือไม่ *CH 06-10-2012
    function otbFTGonCode_onkeydown(ptKeyCode, ptWhich) {
        var tKey = ptKeyCode;
        if (ptWhich != null && ptWhich != '') { tKey = ptWhich; }
        if (tKey == 13) {
            if ($('#otbFTGonCode').val() == $('#FTGonCode').val()) { return !(tKey == 13); }
            W_CHKxHasGon();
            
        }
        $('#otbFTGonCode').val($('#FTGonCode').val());
        return !(tKey == 13);
    }
    function W_CHKxHasGon() {
        var tGonCode = $('#FTGonCode').val();
        if (tGonCode == '') { return false; }
        var tResult = '';
        var tUrl = '<%=Url.Action("C_BRWtGondolaByGonCode", "cvcGondola")%>' + '?ptGonCode=' + tGonCode;
        var oXmlHttp = J002_GEToXMLHttpRequest();
        oXmlHttp.open('GET', encodeURI(tUrl), false);
        oXmlHttp.send(null);
        tResult = oXmlHttp.responseText;
        $('#odiShwErrMsg').html(tResult);
        if (tResult == '') {
            tW_HavGonCode = '1';
        } else {
            tW_HavGonCode = '0';
    ...
    function otbFTGonCode_onblur() {
        if ($('#otbFTGonCode').val() == $('#FTGonCode').val()) {return false; }
        W_CHKxHasGon();
    }
    ...
    //การแสดง Tab หากมีการใช้งานรหัสที่มีการบันทึกแล้วจะไม่ให้เข้า Tab การเลือกสินค้าได้
    function W_SHWxCurrentTab() {
        var nTab = parseInt(tW_HavGonCode);
        J010_SHWxTabData(nTab, 'odiDataEntry', 'odiOptional');
        J009_SHWxTabTop(nTab, 'odiDiv_Toppic1', 'odiDiv_Toppic2')
    }
</script>

    <input type="hidden" id='otbFTGonCode' />
    <input type="hidden" id='otbFTPdtCodeHidden' />
    <% Using Html.BeginForm("C_Create", "cvcGondola", FormMethod.Post, New With {.collection = "cmlGondola", .ptValue = "wGondola", .name = "wGondola"})%>
    <%-------------------ส่วนรายละเอียด-------------------------%>  
    <div class="xW_DivMain xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor" >
            ...
            <div class="xCN_DivMainMenuPage xCN_AllBackgroundColor xCN_AllBorderColor" style='height:525px'>
                <div class="xCN_DivTabToppic xW_DivSize100" >
                    ...
                    <a href="javascript:void(0);" 
                        onclick="W_SHWxCurrentTab();W_SETxGonValue()">
                        ...
                        <div class="xW_DivData">
                            <div class="xW_DivDataSub">
                                <div class="xW_DivCode">
                                    <input type="text"
                                        ...
                                        onkeydown="otbFTGonCode_onkeydown(event.keyCode,event.which)" 
                                        onblur="otbFTGonCode_onblur()"
                                        maxlength = '3' />
                                </div>
                                ...
                                <div class="xW_DivTextBoxWithTxtErr">
                                    ...
                                    <div id='odiShwErrMsg' class='xW_DivShwError'></div>
                                </div>
                                <%--<div class='xW_DivCode'>
                                    <%=Html.ValidationMessageFor(Function(model) model.FTGonCode)%>  
                                </div> --%>        
                            ...
                            <div class="xW_DivDataSub">
                                <div>
                                    ...
                                    <%--
                                    <%=Html.TextBoxFor(Function(model) model.FTGonType, New With {.maxlength = "1", .onkeypress = "return W_CHKBCheckType(event.keyCode,event.which)"})%>
                            ...
                            <div class="xW_DivDataSub">
                                <div>
                                    <%=Html.TextBoxFor(Function(model) model.FTGonName, New With {.maxlength = "50", .style="width:200px", .onkeypress="return J002_CHKbCheckTxtKey(event.keyCode,event.which,false)"})%>

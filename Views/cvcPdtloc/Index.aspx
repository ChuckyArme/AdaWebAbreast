<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMPdtLoc))" %>
...
<script runat="server">
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaMenuPdtLocation") = Resources.resGBGridHeader.olaMenuPdtLocation
        ViewData("olaFindPdtLocation") = Resources.resGBGridHeader.olaFindPdtLocation
        ViewData("olaPdtLocationList") = Resources.resGBGridHeader.olaPdtLocationList
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="xCN_DivMain xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor" >
        <div class="xW_DivSearch">
            <div class='xW_DivPath'>
                <label>\\
                    ...
                    <%=ViewData("olaMenuPdtLocation")%>
                </label>
            </div>
            <div class ="xW_DivSearchCapPrd">
                <label id="olaFilterCaption"><%=ViewData("olaFindPdtLocation")%></label>
            </div>
        ...
        <div class='xCN_DivMainMenuPage xCN_AllBackgroundColor xCN_AllBorderColor' style="height:518px; overflow:auto;">
            <% 
                Dim gridBuilder = Html.Telerik().Grid(Of Adasoft.WebPos.Models.cmlTCNMPdtLoc)(Model)
                ...
                gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
                          .Select("_SelectAjaxEditing", "cvcPdtloc") _
                          .Update("_SaveAjaxEditing", "cvcPdtloc") _
                          .Delete("_DeleteAjaxEditing", "cvcPdtloc", New With {.id = (Function(o) o.FTPlcCode)}) _
                          .Insert("_InsertAjaxEditing", "cvcPdtloc")))
         
                'Add grid columns
                gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPlcCode).Width(150).Title(ViewData("olaCode")) _
                                        .HeaderHtmlAttributes(New With {.name = "FTPlcCode", .style = "text-align:center; font-size:14px;"}) _
                                        .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
                gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPlcName).Width(503).Title(ViewData("olaName")) _
                                        .HeaderHtmlAttributes(New With {.name = "FTPlcName", .style = "text-align:center; font-size:14px;"}) _
                                        .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
              
                gridBuilder.DataKeys(Function(keys) (keys.Add(Function(c) c.FTPlcCode)))
                ...
                Dim commandColumn As New GridActionColumn(Of Adasoft.WebPos.Models.cmlTCNMPdtLoc)(gridBuilder.ToComponent())
                ...
                gridBuilder.RowAction(Function(row) (row.Selected = row.DataItem.FTPlcCode.Equals(ViewData("id"))))
    ...
    <script type="text/javascript">
        ...
        function W_CHKxStatus(e) {
            tW_CmdMode = e.mode;
            document.getElementById('FTPlcCode').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
            document.getElementById('FTPlcName').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
            if (tW_CmdMode == 'edit') {
                J002_SETxReadOnly("FTClrCode", true);
                document.getElementById("FTPlcCode").onkeydown = function() { return false; };
                document.getElementById('FTPlcName').focus();
                document.getElementById('FTPlcName').select();
            } else {
                document.getElementById('FTPlcCode').focus();
            }
        }
        ...
        function W_CHKxUserAuthorize(e) {
            ...
            if (tW_CmdMode == 'insert') {
                ...
                if (tErrMsg != null && tErrMsg != '') {
                    alert(tErrMsg);
                    document.getElementById('FTPlcCode').focus();
                    document.getElementById('FTPlcCode').select();
                    return e.preventDefault();
                }
            }
        }
        function W_CHKxVerifyInsertData() {
            ...
            var tCode = document.getElementById("FTPlcCode").value;
            var tUrl = '<%=Url.Action("C_CHKtVerifyInsertData","cvcPdtLoc") %>' + "?ptDateTimeNow=" + tDataTimeNow + "&ptCode=" + tCode;
        ...
        function W_CHKxUserDeleteAuthorize(e) {
            ...
            if (document.getElementById('FTPlcCode') != null) {
                e.preventDefault();
                return false;
            }
        ...
        function W_SETxFilterOnClick() {
            ...
            $('#Grid tr:has(td) td', this.element).bind('click', function(e) {
                ...
                document.getElementById("olaFilterCaption").innerHTML = '<%=ViewData("olaFindPdtLocation")%>' + ' ' + tCaption;
                var tColName = $('#Grid th').eq(td.cellIndex).attr("name");
                document.getElementById('otbFilterField').value = tColName;
                //set caption
                document.getElementById('otbtextField').value = '<%=ViewData("olaFindPdtLocation")%>' + ' ' + tCaption;
            })
        }
    ...
    <script type ="text/javascript">
         ...
         function W_Show(tValue, tFName, tCaption) {
             ...
             if (tCaption == "" || tCaption == null) {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtLocation")%>';
             }
             else {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtLocation")%>' + ' ' + tCaption;

...
//            return !(window.event && window.event.keyCode == 13);
    ...
    function unhide() {
     var FTCptCode = document.getElementById('FTCptCode').value;
     if (FTCptCode != ""){
        OpenModalDialogRefCode(document.Coupon_Edit.otbOldCptType,document.Coupon_Edit.FTCptCode, document.Coupon_Edit.otbFTCptName, 'C_GETRefName', '50', '50','','<%= AdaWebPos.cCNSP.SP_GETdDateTimeNow  %>');
        }else{
    ...
    <%=""%>
    <% Using Html.BeginForm("C_Edit", "cvcCoupon", FormMethod.Post, New With {.collection = "cmlCoupon", .ptValue = "wCoupon", .name = "wCoupon"})%>
    <%-------------------ส่วนรายละเอียด-------------------------%>  
            ...
            <div class="xCN_AllBorderColor xCN_AllBackgroundColor xCN_DivMainMenuPage" style='height:525px;'>
                ...
                <div class="xW_DivData">
                    <div class="xW_DivDataSub">
                        <div>
                            <%=Html.TextBoxFor(Function(model) model.FTCpnCode, New With {.readonly = "readonly", .maxlength = "30"})%>
                            <%=Html.ValidationMessageFor(Function(model) model.FTCpnCode)%>
                        </div>
                    </div>
                    <div class="xW_DivDataSub">
                        <div>
                            <%=Html.TextBoxFor(Function(model) model.FTCpnBarCode, New With {.maxlength = "30"})%>

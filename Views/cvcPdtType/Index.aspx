<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMPdtType))" %>
...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaMenuPdtType") = Resources.resGBGridHeader.olaMenuPdtType
        ViewData("olaFindPdtType") = Resources.resGBGridHeader.olaFindPdtType
        ViewData("olaPdtTypeList") = Resources.resGBGridHeader.olaPdtTypeList
...
<div class="xCN_DivMain xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor" >
 <%--   <div class="xW_DivToppic xCN_AllMenuFontSize">
         <label><%=ViewData("olaMenuPdtType")%></label>         
    </div>--%>
    <%--<div class="xW_DivToppic xCN_AllMenuFontSize">
        <label>\\<a class="xCN_LabelLink" href="<%=Url.Action("Index","Home") %>"><%=ViewData("olaMenuHome") %></a>\<a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt"><%=ViewData("olaMenuGroup") %></a>\<%=ViewData("olaMenuPdtType")%></label> 
    </div>--%>
    ...
    <div class="xW_DivSearch">
        <div class='xW_DivPath'>
            <label>\\
                ...
                <%=ViewData("olaMenuPdtType")%>
            </label>
        </div>
        <div class ="xW_DivSearchCapPrd">
            <label id="olaFilterCaption"><%=ViewData("olaFindPdtType") %></label>
        </div>
    ...
    <% 
        Dim gridBuilder = Html.Telerik().Grid(Of Adasoft.WebPos.Models.cmlTCNMPdtType)(Model)
        ...
        gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
                  .Select("_SelectAjaxEditing", "cvcPdtType") _
                  .Update("_SaveAjaxEditing", "cvcPdtType") _
                  .Delete("_DeleteAjaxEditing", "cvcPdtType", New With {.id = (Function(o) o.FTPtyCode)}) _
                  .Insert("_InsertAjaxEditing", "cvcPdtType")))
     
            'Add grid columns
            'gridBuilder.Columns(Function(columns) columns.Add(Function(o) o.RowID).Width(100))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPtyCode).Width(150).Title(ViewData("olaCode")) _
                                .HeaderHtmlAttributes(New With {.name = "FTPtyCode", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPtyName).Width(503).Title(ViewData("olaName")) _
                                .HeaderHtmlAttributes(New With {.name = "FTPtyName", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
          
        gridBuilder.DataKeys(Function(keys) (keys.Add(Function(c) c.FTPtyCode)))
        ...
        Dim commandColumn As New GridActionColumn(Of Adasoft.WebPos.Models.cmlTCNMPdtType)(gridBuilder.ToComponent())
        ...
        gridBuilder.RowAction(Function(row) (row.Selected = row.DataItem.FTPtyCode.Equals(ViewData("id"))))
...
<script type="text/javascript">
    ...
    function W_CHKxStatus(e) {
        tW_CmdMode = e.mode;
        document.getElementById('FTPtyCode').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        document.getElementById('FTPtyName').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        if (tW_CmdMode == 'edit') {
            J002_SETxReadOnly("FTPtyCode", true);
            document.getElementById("FTPtyCode").onkeydown = function() { return false; };
            document.getElementById('FTPtyName').focus();
            document.getElementById('FTPtyName').select();
        } else {
            document.getElementById('FTPtyCode').focus();
        }
    }
    ...
    function W_CHKxUserAuthorize(e) {
        ...
        if (tW_CmdMode == 'insert') {
            ...
            if (tErrMsg != null && tErrMsg != '') {
                alert(tErrMsg);
                document.getElementById('FTPtyCode').focus();
                document.getElementById('FTPtyCode').select();
                return e.preventDefault();
            }
        }
    }
    function W_CHKxVerifyInsertData() {
        ...
        var tCode = document.getElementById("FTPtyCode").value;
        var tUrl = '<%=Url.Action("C_CHKtVerifyInsertData","cvcPdtType") %>' + "?ptDateTimeNow=" + tDataTimeNow + "&ptCode=" + tCode;
    ...
    function W_CHKxUserDeleteAuthorize(e) {
        ...
        if (document.getElementById('FTPtyCode') != null) {
            e.preventDefault();
            return false;
        }
     ...
     <script type ="text/javascript">
         ...
         function window_onload() {
             try {
                 ...
                 W_Show('<%=Viewdata("ptWheV")%>', 'FTPtyCode', '<%=ViewData("olaCode")%>');
                 J003_DISxWaiting();
             }
             catch (ex) {
                 J003_DISxWaiting();
         ...
         function W_Show(tValue, tFName, tCaption) {
             ...
             if (tCaption == "" || tCaption == null) {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtType")%>';
             }
             else {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtType")%>' + ' ' + tCaption;
         ...
         function W_FRMxSearchMaster() {
             ...
             if (oField.value != '') {
                 ...
                 //window.location = '<%=Url.Action("Index","cvcPdtType") %>' + '?ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value + '&ptAuthorRead=1';
             } else {

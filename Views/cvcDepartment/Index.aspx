<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMDepart))" %>
...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaMenuDepartment") = Resources.resGBGridHeader.olaMenuDepartment
        ViewData("olaFindDepartment") = Resources.resGBGridHeader.olaFindDepartment
        ViewData("olaDepartmentList") = Resources.resGBGridHeader.olaDepartmentList
...
<div class="xW_DivMain xCN_AllBorderColor xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor">
 <%--   <div class="xW_DivToppic xCN_AllMenuFontSize">
    ...
    <%--<div class="xW_DivToppic xCN_AllMenuFontSize">
        <label>\\<a class="xCN_LabelLink" href="<%=Url.Action("Index","Home") %>"><%=ViewData("olaMenuHome")%></a>\<a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Other"><%=ViewData("olaMenuGroup") %></a>\<%=ViewData("olaMenuDepartment")%></label> 
    </div>--%>
    ...
    <div class="xW_DivSearch">
        <div class='xW_DivPath'>
            <label>\\
                ...
                <%=ViewData("olaMenuDepartment")%>
            </label>
        </div>
        <div class ="xW_DivSearchCapPrd">
            <label id="olaFilterCaption"><%=ViewData("olaFindDepartment")%></label>
        </div>
        ...
        <div class="xCN_DivBrowData xCN_DivBrowDataSetTopMargin xW_DivSearchImg" >
              <img id="Browse"
                  ...
                  alt="<%=ViewData("olaFindDepartment")%>" 
    ...
    <% 
        Dim gridBuilder = Html.Telerik().Grid(Of Adasoft.WebPos.Models.cmlTCNMDepart)(Model)
        ...
        gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
                  .Select("_SelectAjaxEditing", "cvcDepartment") _
                  .Update("_SaveAjaxEditing", "cvcDepartment") _
                  .Delete("_DeleteAjaxEditing", "cvcDepartment", New With {.id = (Function(o) o.FTDptCode)}) _
                  .Insert("_InsertAjaxEditing", "cvcDepartment")))
     
            'Add grid columns
            'gridBuilder.Columns(Function(columns) columns.Add(Function(o) o.RowID).Width(100))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTDptCode).Width(150).Title(ViewData("olaCode")) _
                                .HeaderHtmlAttributes(New With {.name = "FTDptCode", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTDptName).Width(503).Title(ViewData("olaName")) _
                                .HeaderHtmlAttributes(New With {.name = "FTDptName", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
          
        gridBuilder.DataKeys(Function(keys) (keys.Add(Function(c) c.FTDptCode)))
        ...
        Dim commandColumn As New GridActionColumn(Of Adasoft.WebPos.Models.cmlTCNMDepart)(gridBuilder.ToComponent())
        ...
        gridBuilder.RowAction(Function(row) (row.Selected = row.DataItem.FTDptCode.Equals(ViewData("id"))))
...
<script type="text/javascript">
    ...
    function W_CHKxStatus(e) {
        ...
        tW_CmdMode = e.mode;
        document.getElementById('FTDptCode').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        document.getElementById('FTDptName').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        if (tW_CmdMode == 'edit') {
            J002_SETxReadOnly("FTDptCode", true);
            document.getElementById("FTDptCode").onkeydown = function() { return false; };
            document.getElementById('FTDptName').focus();
            document.getElementById('FTDptName').select();
        } else {
            document.getElementById('FTDptCode').focus();
        }
    }
    ...
    function W_CHKxUserAuthorize(e) {
        ...
        if (tW_CmdMode == 'insert') {
            ...
            if (tErrMsg != null && tErrMsg != '') {
                alert(tErrMsg);
                document.getElementById('FTDptCode').focus();
                document.getElementById('FTDptCode').select();
                return e.preventDefault();
            }
        }
    }
    function W_CHKxVerifyInsertData() {
        ...
        var tCode = document.getElementById("FTDptCode").value;
        var tUrl = '<%=Url.Action("C_CHKtVerifyInsertData","cvcDepartment") %>' + "?ptDateTimeNow=" + tDataTimeNow + "&ptCode=" + tCode;
    ...
    function W_CHKxUserDeleteAuthorize(e) {
        ...
        if (document.getElementById('FTDptCode') != null) {
            e.preventDefault();
            return false;
        }
    ...
    function W_SETxFilterOnClick() {
        ...
        $('#Grid tr:has(td) td', this.element).bind('click', function(e) {
            ...
            document.getElementById("olaFilterCaption").innerHTML = '<%=ViewData("olaFindDepartment")%>' + ' ' + tCaption;
            var tColName = $('#Grid th').eq(td.cellIndex).attr("name");
            document.getElementById('otbFilterField').value = tColName;
            //set caption
            document.getElementById('otbtextField').value = '<%=ViewData("olaFindDepartment")%>' + ' ' + tCaption;
        })
    }
     ...
     <script type ="text/javascript">
         ...
         function window_onload() {
             try {
                 ...
                 W_Show('<%=Viewdata("ptWheV")%>', 'FTDptCode', '<%=ViewData("olaCode")%>');
                 J003_DISxWaiting();
             }
             catch (ex) {
                 J003_DISxWaiting();
         ...
         function W_Show(tValue, tFName, tCaption) {
             ...
             if (tCaption == "" || tCaption == null) {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindDepartment")%>';
             }
             else {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindDepartment")%>' + ' ' + tCaption;

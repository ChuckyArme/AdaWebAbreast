...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaGenCode") = AdaWebPos.My.Resources.resLCcvcBranch.olaGenCode
...
<script type="text/javascript" >
    ...
    function window_onload() {
        ...
        J002_SETxReadOnly("otbFTBchCode", true);
        J002_SETxReadOnly("otbFTBchWheStkName", true);
        J002_SETxReadOnly("otbFTZneName", true);
        document.getElementById('FTBchHQ').value = '0';

        if ('<%=ViewData("ovd_tStaChange")%>' == 'Y') { J002_SETxReadOnly("otbFTBchCode", true); }
    ...
    function W_FRMxGenCode() {
        if (tW_StaGenCode == 'N') {
            ...
            J002_SETxReadOnly("otbFTBchCode", false);
    ...
    //Gen Branch Code And Check Branch code In Temp 
    //*CH 08-03-2013
    function W_FRMxGenBchCode() {
        if (tW_StaGenCode == 'N') {
            tW_StaGenCode = 'Y';
            J001_GETtAutoGenCode('cvcBranch', 'C_GETxAutoGenBchCode', document.getElementById('otbFTBchCode'));
            J002_SETxReadOnly("otbFTBchCode", false);
    ...
    function W_FRMxForwClose() {
        ...
        if ((tStaChange == "Y") || (tChageData == "Y")) {
            ...
            switch (tDialogResult) {
                ...
                case oJ003_DialogResult.No: W_DELxClearTemp(); window.location = '<%=Url.Action("Index","cvcBranch") %>'; break;
            }
        } else { W_DELxClearTemp(); window.location = '<%=Url.Action("Index","cvcBranch") %>'; }
        J003_DISxWaiting();
    }
    ...
    function W_DELxClearTemp() {
        var tPostData = 'ptBchCode=' + $('#otbFTBchCode').val();
        var tUrl = '<%= Url.Action("C_DELxClearTemp", "cvcBranch") %>';  //RETURN COMPANY & BRANCH
    ...
    function W_FRMxSaveDocument() {
        ...
        if (bAllow == false) {
            ...
            W_CHKbBchHQ();
            document.wBranch.submit();
        }
    }

    function W_CHKbStockDuplicate() {
        if ($('#otbFTBchWheStk').val() == $('#otbFTBchWheStkOld').val()) { return false; }//*CH 03-10-2012
        ...
        ///cvcBranch/W_CHKbWarehouse
        var tUrl = '<%=Url.Action("W_CHKbWarehouse","cvcBranch") %>?ptStkCode=' + tStkCode + "&ptDate=" + tDate;
    ...
    function otbFTBchWheStk_Onblur() {
        if ($('#otbFTBchWheStk').val() == '') { return false; }//*CH 03-10-2012
        W_FRMxGetFieldName('cvcBranch', 'C_GETtFeildName_WheStk', 'otbFTBchWheStk', 'otbFTBchWheStkName', 'otbFTBchWheStkOld', 'TCNMWaHouse', 'FTWahCode', 'FTWahName', '<%=ViewData("olaFTBchWheStk")%>');
...
<%  Using Html.BeginForm("C_Create", "cvcBranch", FormMethod.Post, New With {.collection = "cmlBranch", .id = "wBranch", .name = "wBranch", .enctype = "multipart/form-data"})%>
...
<div class="xW_DivMain xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor" >
    <div style="display:none">
        ...
        <input type="hidden" id="otbFrmName" name="otbFrmName" value="Create" />
                ...
                <div class="xCN_AllBackgroundColor xW_DivDetailSub">
                    ...
                    <div class="xW_DivData">
                        <div class="xW_DivDataSub">
                            ...
                            <div class="xCN_DivBrowData " style="float:left">
                                <img src="<%=ViewData("oimgGencode")%>" 
                                    onclick="W_FRMxGenBchCode()" 
                        ...
                        <div class="xW_DivDataSub">
                            ...
                            <div class="xW_DivProvince" style="float:left">
                                <input type="text" id="otbFTBchWheStkName" style="width:150px" />
                            </div>
                        </div>  
                        <div class="xW_DivDataSub">
                            <div class="xW_DivCode">
                            ...
                            <div class="xW_DivProvince" style="float:left">
                                <input type="text" id="otbFTZneName" style="width:150px" />
                            </div>
                            <div>
            ...
            try {
                ...
                switch (ptCase.toUpperCase()) {
                    case "L":
                        ...
                        $("#wBranch").attr("action", '<%=Url.Action("C_DATxSaveImage","cvcBranch") %>');
                        break;
                    case "M":
                        tFunc = 'C_DATxSaveImageMap';
                        $("#wBranch").attr("action", '<%=Url.Action("C_DATxSaveImageMap","cvcBranch") %>');
                        break;
                }

                if (tFunc != '') {
                    if ($('#otbFTBchCode').val() == '<%=Session("tChkCode")%>') { W_FRMxGenBchCode(); } //Gen Branch Code
                    $("#wBranch").submit();
                }
            } catch (err) {

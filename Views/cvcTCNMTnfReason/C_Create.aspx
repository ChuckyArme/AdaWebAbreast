...
<script type="text/javascript" >
    ...
    function W_FRMxSaveDocument() {
        ...
        if (bAllow == false) {
            ...
            W_CHKbBchHQ();
            document.wTCNMTnfReason.submit();
        }
    ...
    //*ตรวจสอบว่ามีรหัสเหตุผลอยู่แล้วหรือไม่ *CH 17-10-2012
    function otbFTTrnCode_onkeydown(ptKeyCode, ptWhich) {
        var tKey = ptKeyCode;
        if (ptWhich != null && ptWhich != '') { tKey = ptWhich; }
        if (tKey == 13) {
            if ($('#FTTrnCodeHidden').val() == $('#FTTrnCode').val()) { return !(tKey == 13); }
            W_CHKxHasReason();
            
        }
        $('#otbFTGonCode').val($('#FTTrnCode').val());
        return !(tKey == 13);
    }
    function W_CHKxHasReason() {
        var tTrnCode = $('#FTTrnCode').val();
        if (tTrnCode == '') { return false; }
        var tResult = '';
        var tUrl = '<%=Url.Action("C_CHKtReasonByTrnCode", "cvcTCNMTnfReason")%>' + '?ptTrnCode=' + tTrnCode;
        ...
        if (tResult == '') { $('#FTTrnName').focus(); }
    }
    function otbFTTrnCode_onblur() {
        if ($('#FTTrnCodeHidden').val() == $('#FTTrnCode').val()) { return false; }
        W_CHKxHasReason();
    }
</script>

<%  Using Html.BeginForm("C_Create", "cvcTCNMTnfReason", FormMethod.Post, New With {.collection = "cmlTCNMTnfReason", .ptValue = "wTCNMTnfReason", .name = "wTCNMTnfReason"})%>
    ...
    <div id="odiMenuTop" class="xCN_DivMainMenuPage xCN_AllBorderColor">
          <div id="odiMenu" class="xW_DivMenu">
                ...
                <div id ="odiMenuHome" class="xCN_DivMenuHome" style="float:left;">
                    <a href="javascript:void();" onclick="W_FRMxFormClose()">
                        <div class="xCN_DivMenuNewCaption">
                            <%=ViewData("olaBack")%>
                        </div>
                    </a>
            ...
            <div class="xW_DivDataSub">
                <div class="xW_DivCode">
                    <input type="text" 
                        ...
                        onkeydown="otbFTTrnCode_onkeydown(event.keyCode,event.which)" 
                ...
                <div class="xW_DivCode">
                   <input type="text" 
                        id="FTTrnName" value="<%=model.FTTrnName%>" 

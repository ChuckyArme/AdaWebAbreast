<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMColor))" %>
...
<script runat="server">
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaMenuPdtColor") = Resources.resGBGridHeader.olaMenuPdtColor
        ViewData("olaFindPdtColor") = Resources.resGBGridHeader.olaFindPdtColor
        ViewData("olaPdtColorList") = Resources.resGBGridHeader.olaPdtColorList
...
<div class="xCN_DivMain xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor" >
 <%--   <div class="xW_DivToppic xCN_AllMenuFontSize">
         <label><%=ViewData("olaMenuPdtColor")%></label>         
    </div>--%>
    <%--<div class="xW_DivToppic xCN_AllMenuFontSize">
        <label>\\<a class="xCN_LabelLink" href="<%=Url.Action("Index","Home") %>"><%=ViewData("olaMenuHome") %></a>\<a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt"><%=ViewData("olaMenuGroup") %></a>\<%=ViewData("olaMenuPdtColor")%></label> 
    </div>--%>
    ...
    <div class="xW_DivSearch">
        <div class='xW_DivPath'>
            <label>\\
                ...
                <%=ViewData("olaMenuPdtColor")%>
            </label>
        </div>
        <div class ="xW_DivSearchCapPrd">
            <label id="olaFilterCaption"><%=ViewData("olaFindPdtColor") %></label>
        </div>
    ...
    <% 
        Dim gridBuilder = Html.Telerik().Grid(Of Adasoft.WebPos.Models.cmlTCNMColor)(Model)
        gridBuilder.Name("Grid")
        gridBuilder.Pageable(Function(page) page.PageSize(15))
        '*ปิดไว้ใช้งานปุ่มที่สร้างเอง *CH 18-10-2012
        ...
        gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
                  .Select("_SelectAjaxEditing", "cvcPdtColor") _
                  .Update("_SaveAjaxEditing", "cvcPdtColor") _
                  .Delete("_DeleteAjaxEditing", "cvcPdtColor", New With {.id = (Function(o) o.FTClrCode)}) _
                  .Insert("_InsertAjaxEditing", "cvcPdtColor")))
     
            'Add grid columns
            'gridBuilder.Columns(Function(columns) columns.Add(Function(o) o.RowID).Width(100))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTClrCode).Width(150).Title(ViewData("olaCode")) _
                                .HeaderHtmlAttributes(New With {.name = "FTClrCode", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTClrName).Width(503).Title(ViewData("olaName")) _
                                .HeaderHtmlAttributes(New With {.name = "FTClrName", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
          
        gridBuilder.DataKeys(Function(keys) (keys.Add(Function(c) c.FTClrCode)))
        ...
        Dim commandColumn As New GridActionColumn(Of Adasoft.WebPos.Models.cmlTCNMColor)(gridBuilder.ToComponent())
        ...
        gridBuilder.RowAction(Function(row) (row.Selected = row.DataItem.FTClrCode.Equals(ViewData("id"))))
...
<script type="text/javascript">
    ...
    function W_CHKxStatus(e) {
        tW_CmdMode = e.mode;
        document.getElementById('FTClrCode').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        document.getElementById('FTClrName').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        if (tW_CmdMode == 'edit') {
            J002_SETxReadOnly("FTClrCode", true);
            document.getElementById("FTClrCode").onkeydown = function() { return false; };
            document.getElementById('FTClrName').focus();
            document.getElementById('FTClrName').select();
        } else {
            document.getElementById('FTClrCode').focus();
        }
    }
    ...
    function W_CHKxUserAuthorize(e) {
        ...
        if (tW_CmdMode == 'insert') {
            ...
            if (tErrMsg != null && tErrMsg != '') {
                alert(tErrMsg);
                document.getElementById('FTClrCode').focus();
                document.getElementById('FTClrCode').select();
                return e.preventDefault();
            }
        }
    }
    function W_CHKxVerifyInsertData() {
        ...
        var tCode = document.getElementById("FTClrCode").value;
        var tUrl = '<%=Url.Action("C_CHKtVerifyInsertData","cvcPdtColor") %>' + "?ptDateTimeNow=" + tDataTimeNow + "&ptCode=" + tCode;
    ...
    function W_CHKxUserDeleteAuthorize(e) {
        ...
        if (document.getElementById('FTClrCode') != null) {
            e.preventDefault();
            return false;
        }
    ...
    function W_SETxFilterOnClick() {
        ...
        $('#Grid tr:has(td) td', this.element).bind('click', function(e) {
            ...
            document.getElementById("olaFilterCaption").innerHTML = '<%=ViewData("olaFindPdtColor")%>' + ' ' + tCaption;
            var tColName = $('#Grid th').eq(td.cellIndex).attr("name");
            document.getElementById('otbFilterField').value = tColName;
            //set caption
            document.getElementById('otbtextField').value = '<%=ViewData("olaFindPdtColor")%>' + ' ' + tCaption;
        })
    }
     ...
     <script type ="text/javascript">
         ...
         function window_onload() {
             try {
                 ...
                 W_Show('<%=Viewdata("ptWheV")%>', 'FTClrCode', '<%=ViewData("olaCode")%>');
                 J003_DISxWaiting();
             }
             catch (ex) {
                 J003_DISxWaiting();
         ...
         function W_Show(tValue, tFName, tCaption) {
             ...
             if (tCaption == "" || tCaption == null) {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtColor")%>';
             }
             else {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtColor")%>' + ' ' + tCaption;
         ...
         function W_FRMxSearchMaster() {
             ...
             if (oField.value != '') {
                 ...
                 //window.location = '<%=Url.Action("Index","cvcPdtColor") %>' + '?ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value + '&ptAuthorRead=1';
             } else {

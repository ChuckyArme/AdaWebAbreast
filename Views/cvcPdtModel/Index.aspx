<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMPdtModel))" %>
...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaMenuPdtModel") = Resources.resGBGridHeader.olaMenuPdtModel
        ViewData("olaFindPdtModel") = Resources.resGBGridHeader.olaFindPdtModel
        ViewData("olaPdtModelList") = Resources.resGBGridHeader.olaPdtModelList
...
<div class="xCN_DivMain xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor" >
 <%--   <div class="xW_DivToppic xCN_AllMenuFontSize">
         <label><%=ViewData("olaMenuPdtModel")%></label>         
    </div>--%>
    <%--<div class="xW_DivToppic xCN_AllMenuFontSize">
        <label>\\<a class="xCN_LabelLink" href="<%=Url.Action("Index","Home") %>"><%=ViewData("olaMenuHome") %></a>\<a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt"><%=ViewData("olaMenuGroup") %></a>\<%=ViewData("olaMenuPdtModel")%></label> 
    </div>--%>
    <%--\\<a href="<%=Url.Action("Index","Home") %>">หน้าหลัก</a>\<a href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Pdt">สินค้า</a>\<%=ViewData("olaMenuPdtModel")%>--%>
    ...
    <div class="xW_DivSearch">
        <div class='xW_DivPath'>
            <label>\\
                ...
                <%=ViewData("olaMenuPdtModel")%>
            </label>
        </div>
        <div class ="xW_DivSearchCapPrd">
            <label id="olaFilterCaption"><%=ViewData("olaFindPdtModel")%></label>
        </div>
    ...
    <% 
        Dim gridBuilder = Html.Telerik().Grid(Of Adasoft.WebPos.Models.cmlTCNMPdtModel)(Model)
        ...
        gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
                  .Select("_SelectAjaxEditing", "cvcPdtModel") _
                  .Update("_SaveAjaxEditing", "cvcPdtModel") _
                  .Delete("_DeleteAjaxEditing", "cvcPdtModel", New With {.id = (Function(o) o.FTPmoCode)}) _
                  .Insert("_InsertAjaxEditing", "cvcPdtModel")))
     
            'Add grid columns
            'gridBuilder.Columns(Function(columns) columns.Add(Function(o) o.RowID).Width(100))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPmoCode).Width(150).Title(ViewData("olaCode")) _
                                .HeaderHtmlAttributes(New With {.name = "FTPmoCode", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTPmoName).Width(503).Title(ViewData("olaName")) _
                                .HeaderHtmlAttributes(New With {.name = "FTPmoName", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
          
        gridBuilder.DataKeys(Function(keys) (keys.Add(Function(c) c.FTPmoCode)))
        ...
        Dim commandColumn As New GridActionColumn(Of Adasoft.WebPos.Models.cmlTCNMPdtModel)(gridBuilder.ToComponent())
        ...
        gridBuilder.RowAction(Function(row) (row.Selected = row.DataItem.FTPmoCode.Equals(ViewData("id"))))
...
<script type="text/javascript">
    ...
    function W_CHKxStatus(e) {
        ...
        tW_CmdMode = e.mode;
        document.getElementById('FTPmoCode').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        document.getElementById('FTPmoName').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        if (tW_CmdMode == 'edit') {
            J002_SETxReadOnly("FTPmoCode", true);
            document.getElementById("FTPmoCode").onkeydown = function() { return false; };
            document.getElementById('FTPmoName').focus();
            document.getElementById('FTPmoName').select();
        } else {
            document.getElementById('FTPmoCode').focus();
        }
    }
    ...
    function W_CHKxUserAuthorize(e) {
        ...
        if (tW_CmdMode == 'insert') {
            ...
            if (tErrMsg != null && tErrMsg != '') {
                alert(tErrMsg);
                document.getElementById('FTPmoCode').focus();
                document.getElementById('FTPmoCode').select();
                return e.preventDefault();
            }
        }
    }
    function W_CHKxVerifyInsertData() {
        ...
        var tCode = document.getElementById("FTPmoCode").value;
        var tUrl = '<%=Url.Action("C_CHKtVerifyInsertData","cvcPdtModel") %>' + "?ptDateTimeNow=" + tDataTimeNow + "&ptCode=" + tCode;
    ...
    function W_CHKxUserDeleteAuthorize(e) {
        if (document.getElementById('FTPmoCode') != null) {
            e.preventDefault();
            return false;
        }
    ...
    function W_SETxFilterOnClick() {
        ...
        $('#Grid tr:has(td) td', this.element).bind('click', function(e) {
            ...
            document.getElementById("olaFilterCaption").innerHTML = '<%=ViewData("olaFindPdtModel")%>' + ' ' + tCaption;
            var tColName = $('#Grid th').eq(td.cellIndex).attr("name");
            document.getElementById('otbFilterField').value = tColName;
            //set caption
            document.getElementById('otbtextField').value = '<%=ViewData("olaFindPdtModel")%>' + ' ' + tCaption;
        })
    }
     ...
     <script type ="text/javascript">
         ...
         function window_onload() {
             try {
                 ...
                 W_Show('<%=Viewdata("ptWheV")%>', 'FTPmoCode', '<%=ViewData("olaCode")%>');
                 J003_DISxWaiting();
             }
             catch (ex) {
                 J003_DISxWaiting();
         ...
         function W_Show(tValue, tFName, tCaption) {
             ...
             if (tCaption == "" || tCaption == null) {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtModel")%>';
             }
             else {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindPdtModel")%>' + ' ' + tCaption;
         ...
         function W_FRMxSearchMaster() {
             ...
             if (oField.value != '') {
                 ...
                 //window.location = '<%=Url.Action("Index","cvcPdtModel") %>' + '?ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value + '&ptAuthorRead=1';
             } else {

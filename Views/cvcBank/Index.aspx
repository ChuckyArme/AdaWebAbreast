<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMBank))" %>
<%@ Import Namespace="System.Resources" %>
<%@ Import Namespace="AdaWebPos" %>
<script runat="server">
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        AdaWebPos.My.Resources.resLCcvcBank.Culture = New System.Globalization.CultureInfo(Session("tVB_LastLang").ToString)
        ...
        ViewData("olaFind") = AdaWebPos.My.Resources.resLCcvcBank.olaFind
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        function window_onload() {
            ...
            if ('<%=Viewdata("ptWheV")%>' == '' || '<%=Viewdata("ptWheV")%>' == null) {
                W_Col_click('FTBnkCode',' : <%=ViewData("olaFTBnkCode")%>');
            }
        ...
        function W_FRMxNewDoc() {
            ...
            if (bAllow == false) {
                ...
                window.location = '<%=Url.Action("C_Create","cvcBank")%>';
            }
        }
        ...
        function W_FRMxEditDoc(ptCode) {
            ...
            if (bAllow == false) {
                ...
                J003_SHWxWaiting();
                window.location = '<%=Url.Action("C_Edit","cvcBank")%>' + "?ptValue=" + ptCode;
            }
        }
        ...
        function W_DATxDelete(poForm, pobj) {
            ...
            if (tvalue != '') {
                ...
                if (aObjList == true) {
                    window.location = '<%=Url.Action("C_DeleteAll","cvcBank")%>' + "?ptValue=" + tvalue + '&ptDate=' + tDate;
                    return false;
                }
            }
        }
        ...
        function otbFilterValue_onkeydown(ptKeycode, ptWhich) {
            ...
            if (tKey == 13) {
                ...
                if (oField.value != '') {
                    window.location = '<%=Url.Action("Index","cvcBank")%>' + '?ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value;
                }
                else {
    ...
    //            return !(window.event && window.event.keyCode == 13);
        ...
        function W_FRMxGoToPage(ptPage) {
            ...
            window.location = '<%=Url.Action("Index","cvcBank") %>' + '?ptStartPage=' + ptPage + '&ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value;
        }
    ...
    <script type ="text/javascript" >
        function ProcessDelete(frm1, pobj) {
            ...
            if (tvalue != '') {
                ...
                if (aa == true) {
                    window.location = '<%=Url.Action("C_DeleteAll","cvcBank")%>' + '?ptValue=' + tvalue + '&ptDate=' + future.getFullYear() + future.getMonth() + future.getDate() + future.getHours() + future.getMinutes() + future.getSeconds();
                    return false;
                }
            }
        }
        ...
        <div class='xCN_AllBackgroundColor xCN_AllBorderColor xCN_DivMainMenuPage' style="height:525px; overflow:scroll;">        
            <table style="width:100%;" class="x003Grid_Table xW_Tabel">
                <tr class='xW_ThHeader'>
                    ...
                    <th class="xCN_TableHeaderColorTelerik" style="width:20%" align="center" onclick="W_Col_click('FTBnkCode',' : <%=ViewData("olaFTBnkCode")%>')"><%=ViewData("olaFTBnkCode")%></th>
                    <th class="xCN_TableHeaderColorTelerik" style="width:65%" align="center" onclick="W_Col_click('FTBnkName',' : <%=ViewData("olaFTBnkName")%>')"><%=ViewData("olaFTBnkName")%></th>
                ...
                <tr id="<%=nRow%>" class="xW_TdDetail <%=IIf(nRow Mod 2 = 1, "xCN_TableDefaultRowColorTelerik", "xCN_TableAlternateRowColorTelerik")%> xCN_MouseOver" 
                    ...
                    ondblclick="W_FRMxEditDoc('<%=Html.Encode(item.FTBnkCode)%>')"
                    onmouseover="J014_SETxClassWithMouseOver('<%=nRow%>')" 
                    onmouseout="J014_SETxClassWithMouseOut('<%=nRow%>')">
                    <td style="width:5%" align="center">
                        <input type="checkbox" name='<%=Html.Encode(item.FTBnkCode)%>' id="ockDeletes" value=""  onclick="J013_SETxUnmark(document.getElementById('ocmMark'),('ofm'),this);"/> 
                    </td>  
                    <td style="width:20%" align="center" onclick="W_Col_click('FTBnkCode',' : <%=ViewData("olaFTBnkCode")%>')">
                        <%=Html.Encode(item.FTBnkCode)%>
                    </td>
                    <td style="width:65%" align="left" onclick="W_Col_click('FTBnkName',' : <%=ViewData("olaFTBnkName")%>')">
                        <%=Html.Encode(item.FTBnkName)%>
                    </td>
                    <td style="width:10%" align="center">
                        <img src="<%=ViewData("oimgEdit")%>" 
                            onclick="W_FRMxEditDoc('<%=item.FTBnkCode%>')"

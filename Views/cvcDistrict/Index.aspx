<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMDistrictDisplay))" %>
...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaFTPvnName") = AdaWebPos.My.Resources.resLCcvcDistrict.olaFTPvnName
        ...
        ViewData("olaFind") = AdaWebPos.My.Resources.resLCcvcDistrict.olaFind
...
<script type ="text/javascript" >
    ...
    function window_onload() {
        ...
        if ('<%=Viewdata("ptWheV")%>' == '' || '<%=Viewdata("ptWheV")%>' == null) {
            W_GETxColDetail('FTDstCode',' <%=ViewData("olaFTDstCode")%>');
        }
    ...
    function W_FRMxNewDoc()
    {
       ...
       var bAllow = J002_CHKtCheckAuthorize(tW_Menu, tAuthor, tPwdAllow, tMsgNotAuthorize);
        ...
        {
            window.location = '<%=Url.Action("C_Create","cvcDistrict") %>';
        }
   ...
   function W_FRMxDeleteDoc()
    {
        ...
        {
            W_DATxDelete('wDistrict','ocmMark');
        }
   ...
   function W_FRMxEditDoc(ptCode)
    {
        ...
        {
            window.location = '<%=Url.Action("C_Edit","cvcDistrict") %>' + "?ptValue=" + ptCode;
        }
       ...
       function W_FRMxReadDoc(ptCode)
    {
        ...
        {
            window.location = '<%=Url.Action("C_Details","cvcDistrict") %>' + "?ptValue=" + ptCode;
        }
    ...
    function W_FRMxSearchMaster() {
        ...
        if (oField.value != '') {
            window.location = '<%=Url.Action("Index","cvcDistrict") %>' + '?ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value;
        } else {
    ...
    function W_DATxDelete(poForm, pobj) {
        ...
        if (tvalue != '') {
            ...
            if (aObjList == true) {
                window.location = '<%=Url.Action("C_DeleteAll","cvcDistrict") %>' + "?ptValue=" + tvalue + '&ptDate=' + tDate;
                return false;
            }
        }
    }
    ...
    function W_FRMxGoToPage(ptPage) {
        ...
        window.location = '<%=Url.Action("Index","cvcDistrict") %>' + '?ptStartPage=' + ptPage + '&ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value;
    }
             ...
             <table class='xW_Tabel'>
                 <tr class='xW_ThHeader'>
                        <th class="xCN_TableHeaderColorTelerik" style="width:5%" align ="center">
                            ...
                            onclick="J013_SETxCheckedAll('wDistrict',document.getElementById('ocmMark'));J013_SETxHide(document.getElementById('ocmMark'));"/>
                        </th>  
                        <th class="xCN_TableHeaderColorTelerik" style="width:20%" align="center" onclick="W_GETxColDetail('FTDstCode',' <%=ViewData("olaFTDstCode")%>')"><%=ViewData("olaFTDstCode")%></th>
                        <th class="xCN_TableHeaderColorTelerik" style="width:35%" align="center" onclick="W_GETxColDetail('FTDstName',' <%=ViewData("olaFTDstName")%>')"><%=ViewData("olaFTDstName")%></th>
                        <th class="xCN_TableHeaderColorTelerik" style="width:30%" align="center" onclick="W_GETxColDetail('FTPvnName',' <%=ViewData("olaFTPvnName")%>')"><%=ViewData("olaFTPvnName")%></th>
                ...
                <%  Using Html.BeginForm("#", "#", FormMethod.Get, New With {.id = "wDistrict"})%>
                    <%  Dim nRow As Integer%>
                    ...
                    <% For Each item In Model%>
                          <% nRow += 1%>
                             <tr id="<%=nRow%>" class="xW_TdDetail <%=IIf(nRow Mod 2 = 1, "xCN_TableDefaultRowColorTelerik", "xCN_TableAlternateRowColorTelerik")%> xCN_MouseOver" 
                                ...
                                ondblclick="W_FRMxEditDoc('<%=Html.Encode(item.FTDstCode)%>')"
                                onmouseover="J014_SETxClassWithMouseOver('<%=nRow%>')" 
                                onmouseout="J014_SETxClassWithMouseOut('<%=nRow%>')">
                    
                                <td  style="width:5%;" align ="center">
                                    <input type="checkbox" name=' <%=Html.Encode(item.FTDstCode)%>' id="ockDeletes" value="" onclick="J013_SETxUnmark(document.getElementById('ocmMark'),('wDistrict'),this);"/>
                                </td>

                                <td style="width:20%;" align="center" onclick="W_GETxColDetail('FTDstCode',' <%=ViewData("olaFTDstCode")%>')">
                                     
                                     <%=Html.Encode(item.FTDstCode)%>
                                    
                                </td>
                                <td style="width:35%" align="left" onclick="W_GETxColDetail('FTDstName',' <%=ViewData("olaFTDstName")%>')">
                                    <%=Html.Encode(item.FTDstName)%>
                                </td>
                                <td style="width:30%" align="left" onclick="W_GETxColDetail('FTPvnName',' <%=ViewData("olaFTPvnName")%>')">
                                    <%=Html.Encode(item.FTPvnName)%>
                                </td>
                                 <td style="width:10%" align="center">          
                                          <img src="<%=ViewData("oimgEdit")%>" 
                                                onclick="W_FRMxEditDoc('<%=item.FTDstCode%>')"
                                                style="cursor:pointer"
                                                alt="<%=ViewData("olaCanEdit")%>" 
                                                class="xW_ImgSize"}  />
                                </td>
                                <%--<td style="width:10%" align="center"> 
                                            <img src="<%=ViewData("oimgOpenfile")%>" 
                                                alt="<%=ViewData("olaDetail").ToString%>" 
                                                style="cursor:pointer"
                                                onclick="W_FRMxReadDoc('<%=item.FTDstCode%>')"
                                                class="xW_ImgSize"}  />
                                </td>--%>

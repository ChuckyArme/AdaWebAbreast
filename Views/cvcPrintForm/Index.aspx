<%@ Page Title="" Language="VB" Inherits="System.Web.Mvc.ViewPage(Of Adasoft.WebPos.Models.cmlGetSplOrCst)" %>
...
<script runat="server">
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'ViewData("DocNo") = Session("DocNo")
        'ViewData("DocDate") = Session("DocDate")
        'ViewData("RptName") = Session("RptName")
        'ViewData("SplOrCstCode") = Session("SplCode")
        'ViewData("Lang") = Session("Lang")
        'ViewData("BchCode") = cCNVB.tVB_BchCode
        'ViewData("BarCode") = cCNVB.tVB_ShwBarOrPdt
        'ViewData("Fsh") = cCNVB.tVB_tFsh
        'ViewData("Factor") = cCNVB.tVB_nMnyFactor
...
<script type ="text/javascript" >
    ...
    <!-- #include file='<%=Url.Content("~/Shared/LoadBrowse.js") %>'-->
    <!-- #include file='<%=Url.Content("~/Shared/LoadMasterLostFocus.js") %>'-->
    ...
    window.onload = function() {
        if (window.chrome){
         //ตั้งค่าขนาดหน้าจอใหม่ สำหรับ Chrome
            window.resizeTo(823,424);
        }
            W_SETxDisableSpl();
            W_SETxDisbleBttn();
        //////document.getElementById("loading").style.display = "none"; -->
        var tRptMode ='<%=ViewData("RptMode")%>';
        if (tRptMode=='0'){
            document.getElementById('ordSysDef').checked = true;  
            document.getElementById('otbDefRpt').value = '<%=ViewData("RptDefName")%>';
            document.getElementById('otbUsrDefined').value = '<%=ViewData("RptUsrName")%>';
        }
        else {
 ...
 var tW_CstOrSple = '<%=ViewData("SplOrCstCode")%>';
 var tW_FTSatFedCode ='<%=ViewData("FTSatFedCode")%>';
    
function W_SETxDisbleBttn(){
    var tTbleName ='<%=ViewData("FTSatTblName") %>';
...
function W_FRMxSaveSplorCstData() {
	...
	var tSplOrCst='<%=ViewData("SplOrCstCode")%>';
	var tUrl="";
...
function W_FRMxCloseDoc(){
 ...
 function W_FRMxHandleStateChange() {
  ...
  function W_DATxBrowseDoc(ptObjCode, ptObjDocDate) {
        var tCase = "FindDoc";
        var tDate = J002_GETtTimeStamp ();
        var tDocType ='<%=ViewData("DocType")%>';
        var tBchCode ='<%=ViewData("BchCode")%>';
        var tTblName ='<%=ViewData("FTSatTblName")%>';
...
function W_FRMxGetNewDocNo() {
   // OpenMasterDialog(3,document.getElementById('tBchCode'), document.getElementById('otbDocNo'),'C_BrowseMaster', '600', '600','?ptCase=<%=ViewData("FTSatTblName")%>','<%= AdaWebPos.cCNSP.SP_GETdDateTimeNow  %>',document.getElementById('otbDocDate'),document.getElementById('FTSplOrCstCode'))
    ...
    var tTblName ='<%=ViewData("FTSatTblName")%>';
    var tSplOrCst='<%=ViewData("SplOrCstCode")%>';
    var tGetDataFrom ='<%=ViewData("tGetDataFrom")%>';
    var tDate = J002_GETtTimeStamp();
...
// function W_FRMxBrowseRpt() 
// {
//        var tCase = "FindRpt";
//        var tDate = J002_GETtTimeStamp();
//        var tUrl ="/cvcPrintForm/C_BRWxMaster?ptDate=" + tDate;
//        var oW_XmlHttp =J002_GEToXMLHttpRequest();
//        oW_XmlHttp.open('GET',encodeURI(tUrl),false);
//        oW_XmlHttp.send(null);
//        var tValue = oW_XmlHttp.responseText;
//        document.getElementById('otbUsrDefined').value=tValue;
//       
//}
...
function W_FRMxCheckRptUse (){
 ...
 function W_FRMtSentPar2Rpt(Mode){
    ...
    //var tUrl ='<%=Url.Action("C_PostData","cvcPrintForm") %>' + '?tDocNo='+ tDoc + '&tRptName=' + tRptname + '&nLang=' + nLang + '&tSplCode='+ tSqlCode  + '&tBchCode=' + tBchCode +'&tBarCode=' + tBarCode + '&tFsh='+ tFsh + '&nFactor=' + nFactor + '&tComName=' + tComName + '&tRptMode=' + tRptMode + '&nCountDec=' + nCountDec; //*CH 11-11-11 Old
...
<%--<form id="Index" name="Index"  action="http://192.168.0.59:3000/Report/Frm_SQL_SMBillPi.aspx"  method ="post" >--%>
  ...
  <div class="xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor xW_DivMain xCN_AllBackgroundColor">
    ...
    <div class="xW_DivMainBottom">
        <div class="xW_DivMainBottomLeft">
            <fieldset class="xW_DivMainBottomLeftfieldset xW_FieldSetBorderColor"><legend class="xCN_AllFontFamily xCN_AllFontSize xCN_AllFontGroupColor"><%=ViewData("olaGrpCstOrSpl")%></legend>

...
<%Dim mnuNumber As Integer
        ...
        <% 
            ...
            End If
            If 2 <= mnuNumber Then
                gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTField02).Title(tCaption(1)).Width(nWidth(1)).HtmlAttributes(New With {.align = "left"}).ToString)'.Format("{0:dd/MM/yyyy}")
            End If
            If 3 <= mnuNumber Then
                gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTField03).Title(tCaption(2)).Width(nWidth(2)).HtmlAttributes(New With {.align = "left"}))
            End If
            If 4 <= mnuNumber Then
            ...
            gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
              .Select("ogdMaster_SelectAjaxEditing", tController, New With {.ptDate = "", .ptFilterField = ViewData("FilterField"), .ptFilterValue = ViewData("FilterValue")})))
       ...
       <script type="text/javascript">
            ...
            function W_CHKxLoadPartial(){
                var oFieldName = document.getElementById('otbFieldName');
                if (oFieldName.value != ''){
                    loadPartialView('<%=AdaWebPos.cCNSP.SP_GETdDateTimeNow%>','otbCtrlName','otbFieldName','otbFilter');
                } else {

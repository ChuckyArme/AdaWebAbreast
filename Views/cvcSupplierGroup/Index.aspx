<%@ Page Title="" Language="VB" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage(Of IEnumerable (Of Adasoft.WebPos.Models.cmlTCNMSplGrp))" %>
...
Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ViewData("olaMenuSplGrp") = Resources.resGBGridHeader.olaMenuSplGrp
        ViewData("olaFindSplGrp") = Resources.resGBGridHeader.olaFindSplGrp
        ViewData("olaSplGrpList") = Resources.resGBGridHeader.olaSplGrpList
        ...
        ViewData("olaMenuGroup") = Resources.resGBMenuName.omnMngGenSpl
...
<div class="xW_DivMain xCN_AllBorderColor xCN_AllFontFamily xCN_AllFontSize xCN_AllFontColor">
 <%--   <div class="xW_DivToppic xCN_AllMenuFontSize">
    ...
    <%--<div class="xW_DivToppic xCN_AllMenuFontSize">
        <label>\\<a class="xCN_LabelLink" href="<%=Url.Action("Index","Home") %>"><%=ViewData("olaMenuHome")%></a>\<a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Spl"><%=ViewData("olaMenuGroup")%></a>\<%=ViewData("olaMenuSplGrp")%></label> 
    </div>--%>
    ...
    <div class="xW_DivSearch">
        <div class='xW_DivPath'>
            <label>\\
                ...
                <a class="xCN_LabelLink" href="<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Spl">
                    <%=ViewData("olaMenuGroup") %>
                </a>\
                <%=ViewData("olaMenuSplGrp")%>
            </label>
        </div>
        <div class ="xW_DivSearchCapPrd">
            <label id="olaFilterCaption"><%=ViewData("olaFindSplGrp")%></label>
        </div>
    ...
    <% 
        Dim gridBuilder = Html.Telerik().Grid(Of Adasoft.WebPos.Models.cmlTCNMSplGrp)(Model)
        ...
        gridBuilder.DataBinding(Function(dataBinding) (dataBinding.Ajax() _
                  .Select("_SelectAjaxEditing", "cvcSupplierGroup") _
                  .Update("_SaveAjaxEditing", "cvcSupplierGroup") _
                  .Delete("_DeleteAjaxEditing", "cvcSupplierGroup", New With {.id = (Function(o) o.FTSgpCode)}) _
                  .Insert("_InsertAjaxEditing", "cvcSupplierGroup")))

            'Add grid columns
            'gridBuilder.Columns(Function(columns) columns.Add(Function(o) o.RowID).Width(100))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTSgpCode).Width(150).Title(ViewData("olaCode")) _
                                .HeaderHtmlAttributes(New With {.name = "FTSgpCode", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
        gridBuilder.Columns(Function(columns) columns.Bound(Function(o) o.FTSgpName).Width(503).Title(ViewData("olaName")) _
                                .HeaderHtmlAttributes(New With {.name = "FTSgpName", .style = "text-align:center; font-size:14px;"}) _
                                .HtmlAttributes(New With {.style = "text-align:left; font-size:12px;"}))
          
        gridBuilder.DataKeys(Function(keys) (keys.Add(Function(c) c.FTSgpCode)))
        ...
        Dim commandColumn As New GridActionColumn(Of Adasoft.WebPos.Models.cmlTCNMSplGrp)(gridBuilder.ToComponent())
        ...
        gridBuilder.RowAction(Function(row) (row.Selected = row.DataItem.FTSgpCode.Equals(ViewData("id"))))
...
<script type="text/javascript">
    ...
    function W_CHKxStatus(e) {
        ...
        tW_CmdMode = e.mode;
        document.getElementById('FTSgpCode').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        document.getElementById('FTSgpName').onkeypress = function() { return J002_CHKbCheckTxtKey(event.keyCode, event.which) };
        if (tW_CmdMode == 'edit') {
            J002_SETxReadOnly("FTSgpCode", true);
            document.getElementById("FTSgpCode").onkeydown = function() { return false; };
            document.getElementById('FTSgpName').focus();
            document.getElementById('FTSgpName').select();
        } else {
        document.getElementById('FTSgpCode').focus();
        }
    }
    ...
    function W_CHKxUserAuthorize(e) {
        ...
        if (tW_CmdMode == 'insert') {
            ...
            if (tErrMsg != null && tErrMsg != '') {
                alert(tErrMsg);
                document.getElementById('FTSgpCode').focus();
                document.getElementById('FTSgpCode').select();
                return e.preventDefault();
            }
        }
    }
    function W_CHKxVerifyInsertData() {
        ...
        var tCode = document.getElementById("FTSgpCode").value;
        var tUrl = '<%=Url.Action("C_CHKtVerifyInsertData","cvcSupplierGroup") %>' + "?ptDateTimeNow=" + tDataTimeNow + "&ptCode=" + tCode;
    ...
    function W_CHKxUserDeleteAuthorize(e) {
        if (document.getElementById('FTSgpCode') != null) {
            e.preventDefault();
            return false;
        }
    ...
    function W_SETxFilterOnClick() {
        ...
        $('#Grid tr:has(td) td', this.element).bind('click', function(e) {
            ...
            document.getElementById("olaFilterCaption").innerHTML = '<%=ViewData("olaFindSplGrp")%>' + ' ' + tCaption;
            var tColName = $('#Grid th').eq(td.cellIndex).attr("name");
            document.getElementById('otbFilterField').value = tColName;
            //set caption
            document.getElementById('otbtextField').value = '<%=ViewData("olaFindSplGrp")%>' + ' ' + tCaption;
        })
    }
     ...
     <script type ="text/javascript">
         ...
         function window_onload() {
             try {
                 ...
                 W_Show('<%=Viewdata("ptWheV")%>', 'FTSgpCode', '<%=ViewData("olaCode")%>');                 
                 J003_DISxWaiting();
             }
             catch (ex) {
                 J003_DISxWaiting();
         ...
         function W_Show(tValue, tFName, tCaption) {
             ...
             if (tCaption == "" || tCaption == null) {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindSplGrp")%>';
             }
             else {
                 document.getElementById('olaFilterCaption').innerHTML = '<%=ViewData("olaFindSplGrp")%>' + ' ' + tCaption;
         ...
         function W_FRMxSearchMaster() {
             ...
             if (oField.value != '') {
                 ...
                 //window.location = '<%=Url.Action("Index","cvcSupplierGroup") %>' + '?ptWheF=' + oField.value + '&ptWheV=' + oValue.value + '&ptWheT=' + oText.value + '&ptAuthorRead=1';
             } else {
         ...
         function W_FRMxClose() {
             window.location = '<%=Url.Action("Index","cvcCNPanal") %>?ptMenuGrpName=General_Spl';
         }
    </script>
